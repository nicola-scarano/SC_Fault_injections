...
def get_argparser():
    parser = argparse.ArgumentParser(
        description="Supervised compression for image classification tasks"
    )
    ...
    parser.add_argument(
        "-student_only", action="store_true", help="test the student model only"
    )
    parser.add_argument(
        "-no_dp_eval",
        action="store_true",
...
def train_one_epoch(
    training_box, aux_module, bottleneck_updated, device, epoch, log_freq
):
    ...
    for sample_batch, targets, supp_dict in metric_logger.log_every(
        ...
        if uses_aux_loss:
            ...
            metric_logger.update(
                loss=loss.item(), lr=training_box.optimizer.param_groups[0]["lr"]
            )
        metric_logger.meters["img/s"].update(batch_size / (time.time() - start_time))
        if (torch.isnan(loss) or torch.isinf(loss)) and is_main_process():
            raise ValueError(
                "The training loop was broken due to loss = {}".format(loss)
            )
...
def train(
    ...
    if file_util.check_if_exists(ckpt_file_path):
        best_val_top1_accuracy, _, _ = load_ckpt(
            ckpt_file_path, optimizer=optimizer, lr_scheduler=lr_scheduler
        )
    ...
    aux_module = (
        student_model_without_ddp.get_aux_module()
        if check_if_updatable(student_model_without_ddp)
        else None
    )
    ...
    for epoch in range(args.start_epoch, training_box.num_epochs):
        ...
        if (
            epoch_to_update is not None
            and epoch_to_update <= epoch
            and not bottleneck_updated
        ):
...
def main(args):
    ...
    teacher_model = (
        load_model(teacher_model_config, device, distributed)
        if teacher_model_config is not None
        else None
    )
    student_model_config = (
        models_config["student_model"]
        if "student_model" in models_config
    ...
    if not args.test_only:
        ...
        load_ckpt(
            student_model_config["ckpt"], model=student_model_without_ddp, strict=True
        )
    ...
    print(student_model)
    import compressai
    Embeddings = nvbitDNN.extract_statistics_nvbit(
        student_model,
        [torch.nn.Conv2d, torch.nn.Linear, torch.nn.BatchNorm2d, compressai.layers.gdn.GDN1],
        lyr_num=[-1],
        batch_size=1,
