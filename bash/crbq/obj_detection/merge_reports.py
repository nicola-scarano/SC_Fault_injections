...
def merge_fsim_reports(path):
    ...
    if path:
        ...
        #index=((fsim_report['gold_iou@1'].isna()==False) | (fsim_report['gold_ACC@k'].isna()==False))
        # index=(fsim_report['gold_iou@1'].isna()==0.0)
        ...
        df_pivot = df_pivot.pivot_table(index=['FaultID', 'imID', 'G_lab', 'g_bb'],
                          values=['f_bb','F_lab','T_count','F_count','G_count','iou score', 'area_ratio','f_candidate_conf', 'G_score', 'confidence_covered'])
