...
def fix_model(student_model, bounding_ranges):
    state_dict = student_model.state_dict()
    student_module = ModifiedStudent(student_model, bounding_ranges)
    new_state_dict = dict()

    for key in list(state_dict.keys()):
        if key.startswith('features.bottleneck_layer.encoder'):

            fields = key.split('.')
            if fields[3] == '3':
                # logger.info(key)
                k_new = 'features.bottleneck_layer.encoder.4.'+ fields[4]
                previsional_k = 'features.bottleneck_layer.encoder.5.weight'
                new_state_dict[previsional_k] = state_dict[k_new]
                new_state_dict[k_new] = state_dict[key]
                
            elif fields[3] == '4' and not key.endswith('.4.weight'):
                k_new = 'features.bottleneck_layer.encoder.5.'+ fields[4]
                new_state_dict[k_new] = state_dict[key]
            elif not key.endswith('.4.weight'):
    ...
    student_module.load_state_dict(new_state_dict, strict=False)
    return student_module

    

class ModifiedStudent(torch.nn.Module):
    def __init__(self, weak_model, bounding_ranges):
        super().__init__()
        self.bounding_act = bounding_ranges
        self.weak_model = weak_model
        self.reinf_model = deepcopy(weak_model)
        
        self.modify_conv_block()


    def modify_conv_block(self):
        act_counter = 0
        current_max = self.bounding_act[act_counter]
        tmp = list(self.reinf_model.features.bottleneck_layer.encoder.children())
        void = list()
        for el in tmp:
            void.append(el)
            if not isinstance(el, torch.nn.Conv2d) and not isinstance(el, torch.nn.Hardtanh) and act_counter !=0:
                void.append(torch.nn.Hardtanh(min_val=0, max_val=20)) # # min val e max val non sono giusti
            elif isinstance(el, torch.nn.ReLU):
                void.append(torch.nn.Hardtanh(min_val=0, max_val=20))
                act_counter+=1
                current_max = self.bounding_act[act_counter]
        self.weak_model.features.bottleneck_layer.encoder = torch.nn.Sequential(*void)
...
def evaluate(model_wo_ddp, data_loader, device, device_ids, distributed, no_dp_eval=False,
    ...
    if fsim_enabled==True:
        ...
        f1_1 = MulticlassF1Score(task='multiclass', num_classes=100, average='macro')
        rec_1 = MulticlassRecall(average='macro', num_classes=100)
        prec_1 = MulticlassPrecision(average='macro', num_classes=100)

        best_f1 = f1_1(val_distr, val_targ)
        best_rec = rec_1(val_distr, val_targ)
        best_prec = prec_1(val_distr, val_targ)

        f1_k = MulticlassF1Score(task='multiclass', num_classes=100, average='macro', top_k=5)
        rec_k = MulticlassRecall(num_classes=100, average='macro', top_k=5)
        prec_k = MulticlassPrecision(num_classes=100, average='macro', top_k=5)
...
def main(args):
    ...
    # load_ckpt(student_model_config['ckpt'], model=ranger_student, strict = False)
    ranger_student = fix_model(student_model, [20,10])
    # ranger_student = fix_model(student_model, [20,10])
    if args.log_config:
        logger.info(config)

    if not args.test_only:
        train(teacher_model, ranger_student, dataset_dict, ckpt_file_path, device, device_ids, distributed, config, args)
        ranger_student_without_ddp =\
            ranger_student.module if module_util.check_if_wrapped(ranger_student) else ranger_student
        load_ckpt(student_model_config['ckpt'], model=ranger_student_without_ddp, strict=True)
    ...
    if check_if_updatable(ranger_student):
        ranger_student.update()
        

    if check_if_analyzable(ranger_student):
        ranger_student.activate_analysis()
    ...
    if args.fsim_config:
        ...
        cwd=os.getcwd()
        # ranger_student = deepcopy(student_model)
        ranger_student.eval() 
        ...
        evaluate(ranger_student, dataloader, device, device_ids, distributed, no_dp_eval=no_dp_eval,
        ...
        FI_setup.FI_framework.create_fault_injection_model(device,ranger_student,
                                            batch_size=1,
