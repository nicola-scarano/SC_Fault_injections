...
from pytorchfi.FI_Weights import *
...
def main(args):
    ...
    # Fault injection to the Student segmentation Model
    ...
    full_log_path=os.path.join(cwd,"FSIM_logs")
    ...
    evaluate(student_model, test_data_loader, device, device_ids, distributed, num_classes=num_classes,
             no_dp_eval=no_dp_eval, log_freq=log_freq, title='[Student: {}]'.format(student_model_config['name']),header='Golden', fsim_enabled=False, Fsim_setup=FI_setup) 
    ...
    FI_setup.generate_fault_list(flist_mode='sbfm',f_list_file='fault_list.csv',layer=4)    
    ...
    for fault,k in FI_setup.iter_fault_list():
        ...
        try:   
            # 5.2 run the inference with the faulty model 
             evaluate(FI_setup.FI_framework.faulty_model, test_data_loader, device, device_ids, distributed, num_classes=num_classes,
             no_dp_eval=no_dp_eval, log_freq=log_freq, title='[Student: {}]'.format(student_model_config['name']),header='FSIM', fsim_enabled=True, Fsim_setup=FI_setup) 
